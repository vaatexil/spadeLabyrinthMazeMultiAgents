...
class PeriodicSenderScout(Agent):
    ...
    class InformBehav(PeriodicBehaviour):
        def init(self, maze, eel, mr):
            ...
            self.directionsDoors = [[0, 1], [-1, 0], [0, -1], [1, 0]] # position of the doors relatively to the direction taken
            for i in range (0,width): # we search our agent in the labyrinth
                for j in range (0,width):
                    if(maze.maze[i][j] == self.id):
                        self.position = [i,j]
        ...
        def calcNewPos(self):
            ...
            self.position[0] += self.directions[self.direction][0] # update of the position
        ...
        def searchDoor(self): # search if there is a door nearby the agent
            if(self.maze.maze[self.position[0] + self.directionsDoors[self.direction][0]][self.position[1] + self.directionsDoors[self.direction][1]][0] == 2):
                return (self.position[0] + self.directionsDoors[self.direction][0],self.position[1] + self.directionsDoors[self.direction][1]) # It means there is a door nearby in an adjacent cell
            return 0
